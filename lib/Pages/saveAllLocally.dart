    ...
    try {
      ...
      if (userId != null) {
        ...
        for (QueryDocumentSnapshot item in userItems.docs) {
          ...
          String? localImagePath = await getImageLocalPath(itemId);
          ImageProvider<Object>? imageProvider;

          if (localImagePath != null) {
            // Image exists locally, use local image path
            imageProvider = FileImage(File(localImagePath));
          } else {
            // Image does not exist locally, fetch from network
                  ...
                  onPressed: () async {
                    try {
                      if (imageFile != null &&
                        ...
                        if (localImagePath != null) {
                          // Upload the image to Firebase Storage with the same custom filename
                          String? imageUrl = await uploadImageToStorage(
                              File(localImagePath), userId!, fileName);
